module parameters
	...
	integer, parameter :: TimeInterval=1, MaxTime=301,CycleRecord=1000	!number of interval for each txt output
	real, parameter :: AtomsAddedPerCycle =0.07, TempIncPerCycle =0.5  
    !how many time will the time pass

	real(kind=8), parameter::Ed=0.400			!diffusion energy
	real(kind=8), parameter::Eb=0.050			!bonding energy
	real(kind=8), parameter::Tc=300   			!temp.

    real(kind=8), parameter::fc=1E8		!constant frequency
